<?php
///@cond INTERNAL
use app\decibel\debug\DDebug;
use app\decibel\debug\DDebuggingInformation;
use app\decibel\debug\DError;
use app\decibel\debug\DErrorHandler;

$errorCount = DErrorHandler::$errorCount;
$debugging = DErrorHandler::$debugging;
$profiling = DErrorHandler::$profiling;
if (count($debugging) > 0) {
    $plural = ($errorCount != 1) ? 's' : '';
    $message = " ({$errorCount} Application Error{$plural})";
} else {
    $message = '';
}
$popup = <<<EOD
<!DOCTYPE html>
<html>
<head>
	<title>Decibel Debug Console</title>
	<link type="text/css" rel="stylesheet" href="/theme/styles/Debug.css" media="all" />
	<script type="text/javascript" src="/theme/jquery/jquery.js"></script>
	<script type="text/javascript" src="/theme/jquery/jquery.tablesorter.js"></script>
	<script type="text/javascript" src="/theme/jquery/jquery.snippet.js"></script>
</head>
<body class="app-decibel-debug-debugconsole">
	<div class="app-decibel-notification">
		<img src="data:image/png;base64," style="float: left;height: 30px; margin: 4px;" />
		<div class="inner">
			 <span style="float: right;">Decibel Debug Console{$message}<span>
		</div>
	</div>
EOD;
if (count($debugging) > DErrorHandler::DEBUG_SESSION_LIMIT) {
    $popup .= <<<EOD
	<div class="app-decibel-debug-derror">
		<div>
			<strong>Debug limit reached!</strong><br />
			Not all debugs will be shown.
		</div>
	</div>
EOD;
}
if (count($debugging)) {
    foreach ($debugging as $debug) {
        $popup .= printDebug($debug);
    }
}
if (count($profiling)) {
    foreach ($profiling as $report) {
        $popup .= str_replace('\\', '&#92;', $report);
    }
}
$popup .= <<<EOD
	</body>
	<script type="text/javascript">
		$('table').tablesorter();
		$('.sql').snippet("sql", {style: 'emacs', showNum: false, menu: false});
		$('.html').snippet("html", {style: 'emacs', menu: false});
	</script>
</html>
EOD;
$popup = addslashes($popup);
$popup = preg_replace(
    array('/[\r\n\t]/', '/<\/script>/is', '/<script/is'),
    array(' ', "</scr'+'ipt>", "<scr'+'ipt"),
    $popup
);
$output = <<<EOD
		<script type="text/javascript">
		//<![CDATA[
			var title = (self.name == '') ? 'DecibelDebugConsole' : 'DecibelDebugConsole_' + self.name;
			_debug_console = window.open('', title, 'width=680,height=600,resizable,scrollbars=yes');
			_debug_console.document.write('{$popup}');
			_debug_console.document.close();
		//]]>
		</script>
EOD;
function printDebug(DDebuggingInformation $debug)
{
    $file = str_replace('\\', '&#92;', $debug->getFile());
    $msg = str_replace('\\', '&#92;', $debug->getMessage());
    $backtrace = $debug->getBacktrace();
    if ($backtrace) {
        $trace = nl2br(str_replace('\\', '&#92;', $backtrace));
        $trace = "<strong>Backtrace</strong>:<br /><span class=\"App_Error_trace\">{$trace}</span><br /><br />";
    } else {
        $trace = '';
    }
    if ($debug instanceof DError) {
        $class = 'app-decibel-debug-derror';
        $type = $debug->getTypeName();
    } else {
        if ($debug instanceof DDebug) {
            $class = 'app-decibel-debug-ddebug';
            $msg = "<pre>{$msg}</pre>";
            $type = 'Debug';
        }
    }

    return <<<EOD
<div class="{$class}">
	<div>
		<strong><u>{$type}</u></strong> ({$file}, line {$debug->getLine()})<br /><br />
		{$msg}<br /><br />
		{$trace}
	</div>
</div>
EOD;
}

///@endcond
